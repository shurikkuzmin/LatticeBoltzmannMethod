...
{
	std::string filename = "inamuro/" + fname+".dat";
...
void update_inamuro()
{
	for (int counter=0; counter<bb_nodes.size(); counter++)
	{
		...
		double conc_neg = 0.0;
		for (int k = 0; k < antidirs[counter].size(); ++k)
		{
			char antidir = antidirs[counter][k];
			conc_neg += f[bb_nodes[counter]*NPOP + antidir];
		}

		double weights_pos = 0.0;
		for (int k = 0; k < dirs[counter].size(); k++)
		{
			char dir=dirs[counter][k];
			weights_pos += weights[dir];
		}

		double conc_prime = (conc_wall - conc_neg) / weights_pos;

		for (int k = 0; k < dirs[counter].size(); ++k)
		{
			char dir = dirs[counter][k];
			f[bb_nodes[counter]*NPOP + dir] = weights[dir] * conc_prime;
		}
	}
...
void initialize_geometry()
{
    ...
    antidirs = new std::vector<char>[bb_nodes.size()];
    for(int counter=0;counter<bb_nodes.size();counter++)
	{
		antidirs[counter].push_back(0);
		for(int k=1;k<NPOP;k++)
		{
			...
			if (geometry[counter2]==1)
				...
				antidirs[counter].push_back(k);
		}
		std::cout << "Node: " << bb_nodes[counter] << " " << dirs[counter].size() << " " << antidirs[counter].size() << std::endl; 
	}
}
