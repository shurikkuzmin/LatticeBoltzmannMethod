...
template<typename D> void Solver<D>::updatePhase()
{
	...
	if (size!=1)
	{
                 ...
                 int top_rank=(rank+size+1)%size;
                 int bottom_rank=(rank+size-1)%size;
   
                MPI_Status status; 
                MPI_Request request;
                MPI_Isend(lattice->phase_bottom,NX*NY,MPI_DOUBLE,bottom_rank,1,MPI_COMM_WORLD,&request);
                MPI_Recv(lattice->phase_temp,NX*NY,MPI_DOUBLE,top_rank,1,MPI_COMM_WORLD,&status);
               MPI_Wait(&request,&status);
		
                MPI_Isend(lattice->phase_top,NX*NY,MPI_DOUBLE,top_rank,1,MPI_COMM_WORLD,&request);
                MPI_Recv(lattice->phase_bottom,NX*NY*D::NPOP,MPI_DOUBLE,bottom_rank,1,MPI_COMM_WORLD,&status);
               MPI_Wait(&request,&status);
               double * temp;
               temp=lattice->phase_top;
               lattice->phase_top=lattice->phase_temp;
               lattice->phase_temp=temp;
...
template<typename D> void Solver<D>::propagate()
{
    ...
    MPI_Status status;
                ...
                MPI_Request request;
                MPI_Isend(lattice->layer_bottom_f,NX*NY*D::NPOP,MPI_DOUBLE,bottom_rank,1,MPI_COMM_WORLD,&request);
                MPI_Recv(lattice->layer_temp,NX*NY*D::NPOP,MPI_DOUBLE,top_rank,1,MPI_COMM_WORLD,&status);
               MPI_Wait(&request,&status);
		
                MPI_Isend(lattice->layer_top_f,NX*NY*D::NPOP,MPI_DOUBLE,top_rank,1,MPI_COMM_WORLD,&request);
                MPI_Recv(lattice->layer_bottom_f,NX*NY*D::NPOP,MPI_DOUBLE,bottom_rank,1,MPI_COMM_WORLD,&status);
               MPI_Wait(&request,&status);
               double * temp;
               temp=lattice->layer_top_f;
               lattice->layer_top_f=lattice->layer_temp;
               lattice->layer_temp=temp;

                MPI_Isend(lattice->layer_bottom_g,NX*NY*D::NPOP,MPI_DOUBLE,bottom_rank,1,MPI_COMM_WORLD,&request);
                MPI_Recv(lattice->layer_temp,NX*NY*D::NPOP,MPI_DOUBLE,top_rank,1,MPI_COMM_WORLD,&status);
               MPI_Wait(&request,&status);
		
                MPI_Isend(lattice->layer_top_g,NX*NY*D::NPOP,MPI_DOUBLE,top_rank,1,MPI_COMM_WORLD,&request);
                MPI_Recv(lattice->layer_bottom_g,NX*NY*D::NPOP,MPI_DOUBLE,bottom_rank,1,MPI_COMM_WORLD,&status);
               MPI_Wait(&request,&status);
               temp=lattice->layer_top_g;
               lattice->layer_top_g=lattice->layer_temp;
               lattice->layer_temp=temp;
